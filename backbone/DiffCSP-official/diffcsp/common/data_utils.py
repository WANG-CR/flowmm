...
def save_hdf5_file(hamiltonian_mapping, file_path):
    """
    Save a Hamiltonian mapping to an HDF5 file.

    Args:
        hamiltonian_mapping (dict): A dictionary where the keys are tuples
                                    (edge_index + to_jimages) and the values
                                    are PyTorch tensors (Hamiltonian matrices).
    ...
    with h5py.File(file_path, "w") as fid:
        for key, value in hamiltonian_mapping.items():
            # Convert the key tuple to a JSON string
            json_key = json.dumps(key)
            
            # Save the tensor as an HDF5 dataset
            fid.create_dataset(json_key, data=value.numpy())
